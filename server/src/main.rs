...
async fn get_user_role(state: &AppState, token: &str) -> String {
    ...
    match res {
        Ok(r) if r.status().is_success() => {
            ...
            json.get("role").and_then(|v| v.as_str()).unwrap_or("client").to_string()
        }
        _ => "client".to_string(),
    }
}
...
fn user_layout(title: &str, body: &str) -> String { layout(title, body, r#"<a href="/qr">QR</a><a href="/convert">Convert</a><a href="/notepad">Notepad</a><a href="/upload">Upload</a><a href="/my-links">My Links</a><a href="/my-notepads">My Notes</a><a href="/my-uploads">My Files</a><a href="/logout">Logout</a>"#) }
fn admin_layout(title: &str, body: &str) -> String { layout(title, body, r#"<a href="/qr">QR</a><a href="/convert">Convert</a><a href="/notepad">Notepad</a><a href="/upload">Upload</a><a href="/admin">Admin</a><a href="/logout">Logout</a>"#) }
...
fn admin_dashboard_html(stats: &serde_json::Value) -> String {
    let total_links = stats.get("total_links").and_then(|v| v.as_i64()).unwrap_or(0);
    let total_notepads = stats.get("total_notepads").and_then(|v| v.as_i64()).unwrap_or(0);
    let total_uploads = stats.get("total_uploads").and_then(|v| v.as_i64()).unwrap_or(0);
    let total_users = stats.get("total_users").and_then(|v| v.as_i64()).unwrap_or(0);

    admin_layout("Admin Dashboard", &format!(r#"<div class="card" style="max-width:900px;margin:2rem auto"><h1>👑 Admin Dashboard</h1><div class="grid mt-2"><div class="card text-center"><h3>🔗 Links</h3><p style="font-size:2rem;font-weight:bold">{}</p><a href="/admin/links" class="btn btn--sm">Manage</a></div><div class="card text-center"><h3>📝 Notepads</h3><p style="font-size:2rem;font-weight:bold">{}</p><a href="/admin/notepads" class="btn btn--sm">Manage</a></div><div class="card text-center"><h3>📁 Uploads</h3><p style="font-size:2rem;font-weight:bold">{}</p><a href="/admin/uploads" class="btn btn--sm">Manage</a></div><div class="card text-center"><h3>👥 Users</h3><p style="font-size:2rem;font-weight:bold">{}</p><a href="https://db.w9.nu/admin" class="btn btn--sm" target="_blank">Manage in W9-DB</a></div></div></div>"#,
...
fn admin_links_html(links: &[serde_json::Value], msg: Option<&str>) -> String {
    ...
    admin_layout("Admin Links", &format!(r#"<div class="card" style="max-width:1100px;margin:2rem auto"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem"><h1 style="margin:0">👑 Admin - All Links</h1><a href="/admin" class="btn btn--sm">← Dashboard</a></div>{}{}</div>
    <script>
...
fn admin_notepads_html(notepads: &[serde_json::Value], msg: Option<&str>) -> String {
    ...
    let html = if notepads.is_empty() {
        r#"<div class="card text-center mt-2"><p class="text-muted">No notepads found.</p></div>"#.to_string()
    } else {
        ...
        format!(r#"<div class="card mt-2" style="overflow-x:auto"><table><thead><tr><th>Code</th><th>Title</th><th>Owner</th><th>Created</th><th>Expires</th><th>Actions</th></tr></thead><tbody>{}</tbody></table></div>"#, rows.join(""))
    };
    admin_layout("Admin Notepads", &format!(r#"<div class="card" style="max-width:1000px;margin:2rem auto"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem"><h1 style="margin:0">📝 Admin - All Notepads</h1><a href="/admin" class="btn btn--sm">← Dashboard</a></div>{}{}</div>
    <script>
...
fn admin_uploads_html(uploads: &[serde_json::Value], msg: Option<&str>) -> String {
    ...
    let html = if uploads.is_empty() {
        r#"<div class="card text-center mt-2"><p class="text-muted">No uploads found.</p></div>"#.to_string()
    } else {
        ...
        format!(r#"<div class="card mt-2" style="overflow-x:auto"><table><thead><tr><th>File</th><th>Owner</th><th>Size</th><th>Uploaded</th><th>Actions</th></tr></thead><tbody>{}</tbody></table></div>"#, rows.join(""))
    };
    admin_layout("Admin Uploads", &format!(r#"<div class="card" style="max-width:900px;margin:2rem auto"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem"><h1 style="margin:0">📁 Admin - All Uploads</h1><a href="/admin" class="btn btn--sm">← Dashboard</a></div>{}{}</div>
    <script>
...
fn my_notepads_html(notepads: &[serde_json::Value], msg: Option<&str>) -> String {
    ...
    let html = if notepads.is_empty() {
        ...
        format!(r#"<div class="card mt-2" style="overflow-x:auto"><table><thead><tr><th>Code</th><th>Title</th><th>Created</th><th>Expires</th><th>Actions</th></tr></thead><tbody>{}</tbody></table></div>"#, rows.join(""))
    };
    user_layout("My Notepads", &format!(r#"<div class="card" style="max-width:800px;margin:2rem auto"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem"><h1 style="margin:0">📝 My Notepads</h1><a href="/notepad" class="btn">+ New Note</a></div>{}{}</div>
    <script>
...
fn my_uploads_html(uploads: &[serde_json::Value], msg: Option<&str>) -> String {
    ...
    let html = if uploads.is_empty() {
        ...
        format!(r#"<div class="card mt-2" style="overflow-x:auto"><table><thead><tr><th>File</th><th>Size</th><th>Uploaded</th><th>Actions</th></tr></thead><tbody>{}</tbody></table></div>"#, rows.join(""))
    };
    user_layout("My Files", &format!(r#"<div class="card" style="max-width:800px;margin:2rem auto"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem"><h1 style="margin:0">📁 My Files</h1><a href="/upload" class="btn">+ Upload</a></div>{}{}</div>
    <script>
...
async fn file_convert_post(jar: CookieJar, state: State<AppState>, mut multipart: Multipart) -> impl IntoResponse {
    ...
    while let Some(field) = multipart.next_field().await.unwrap_or(None) {
        let name = field.name().unwrap_or("").to_string();
        match name.as_str() {
            "format" => { format = field.text().await.unwrap_or_default(); }
            "file" => { file_data = Some(field.bytes().await.unwrap_or_default().to_vec()); }
            _ => {}
        }
    }
    let data = match file_data { Some(d) => d, None => return Html(file_convert_html()).into_response() };
    ...
    let mime = match ext { "jpg" => "image/jpeg", "webp" => "image/webp", _ => "image/png" };
    (StatusCode::OK, [("Content-Type", mime), ("Content-Disposition", &format!("attachment; filename=\"{}\"", new_name))], buf).into_response()
}
...
async fn create_link(jar: CookieJar, state: State<AppState>, Form(form): Form<LinkCreateReq>) -> impl IntoResponse {
    ...
    if let Some(ref code) = form.code { payload["code"] = serde_json::Value::String(code.clone()); }
    ...
    match state.http_client.post(&api_url).json(&payload).send().await {
        Ok(resp) if resp.status().is_success() => {
            ...
            let id = Uuid::new_v4();
            let expires_at = if form.expires_hours.unwrap_or(0) > 0 { Some(chrono::Utc::now() + chrono::Duration::hours(form.expires_hours.unwrap_or(24))) } else { None };
            let _ = state.db.execute(
...
async fn edit_link_page(jar: CookieJar, state: State<AppState>, axum::extract::Path(id): axum::extract::Path<String>) -> impl IntoResponse {
    ...
    let expires_hours = expires_at.map(|exp| (exp - chrono::Utc::now()).num_hours().max(0)).unwrap_or(0);
...
async fn delete_link(jar: CookieJar, state: State<AppState>, axum::extract::Path(id): axum::extract::Path<String>) -> impl IntoResponse {
    let user = match require_auth(&jar, &state).await { Some(u) => u, None => return (StatusCode::UNAUTHORIZED, "Unauthorized").into_response() };
    let email = user.get("email").and_then(|v| v.as_str()).unwrap_or("").to_string();
    match state.db.execute("DELETE FROM links WHERE id::text = $1 AND owner_email = $2", &[&id, &email]).await {
...
async fn delete_notepad(jar: CookieJar, state: State<AppState>, axum::extract::Path(id): axum::extract::Path<String>) -> impl IntoResponse {
    let user = match require_auth(&jar, &state).await { Some(u) => u, None => return (StatusCode::UNAUTHORIZED, "Unauthorized").into_response() };
    let email = user.get("email").and_then(|v| v.as_str()).unwrap_or("").to_string();
    match state.db.execute("DELETE FROM notes WHERE id::text = $1 AND owner_email = $2", &[&id, &email]).await {
        Ok(1) => (StatusCode::OK, "Deleted").into_response(),
...
async fn my_uploads_page(jar: CookieJar, state: State<AppState>) -> impl IntoResponse {
    ...
    let uploads: Vec<serde_json::Value> = rows.iter().map(|row| {
        serde_json::json!({
...
async fn delete_upload(jar: CookieJar, state: State<AppState>, axum::extract::Path(id): axum::extract::Path<String>) -> impl IntoResponse {
    let user = match require_auth(&jar, &state).await { Some(u) => u, None => return (StatusCode::UNAUTHORIZED, "Unauthorized").into_response() };
    let email = user.get("email").and_then(|v| v.as_str()).unwrap_or("").to_string();
    match state.db.query_opt("SELECT filename FROM uploads WHERE id::text = $1 AND owner_email = $2", &[&id, &email]).await {
        Ok(Some(r)) => {
            let filename: String = r.get("filename");
            let path = format!("{}/{}", UPLOADS_DIR, filename);
            let _ = tokio::fs::remove_file(&path).await;
            match state.db.execute("DELETE FROM uploads WHERE id::text = $1 AND owner_email = $2", &[&id, &email]).await {
                Ok(1) => (StatusCode::OK, "Deleted").into_response(),
...
async fn admin_dashboard(state: State<AppState>, jar: CookieJar) -> impl IntoResponse {
    let _user = match require_admin(&jar, &state).await { Some(u) => u, None => return Redirect::to("/").into_response() };
    let total_links = state.db_links.query_one("SELECT COUNT(*)::bigint FROM short_links", &[]).await.ok().map(|r| r.get::<_, i64>(0)).unwrap_or(0);
    let total_notepads = state.db.query_one("SELECT COUNT(*)::bigint FROM notes", &[]).await.ok().map(|r| r.get::<_, i64>(0)).unwrap_or(0);
    let total_uploads = state.db.query_one("SELECT COUNT(*)::bigint FROM uploads", &[]).await.ok().map(|r| r.get::<_, i64>(0)).unwrap_or(0);
...
async fn admin_delete_link(jar: CookieJar, state: State<AppState>, axum::extract::Path(id): axum::extract::Path<String>) -> impl IntoResponse {
    let _user = match require_admin(&jar, &state).await { Some(u) => u, None => return (StatusCode::FORBIDDEN, "Admin only").into_response() };
    // Get code first
    match state.db_links.query_opt("SELECT code FROM short_links WHERE id::text = $1", &[&id]).await {
        Ok(Some(r)) => {
            let code: String = r.get("code");
            let _ = state.db_links.execute("DELETE FROM short_links WHERE id::text = $1", &[&id]).await;
            let _ = state.db.execute("DELETE FROM links WHERE code = $1", &[&code]).await;
...
async fn admin_delete_notepad(jar: CookieJar, state: State<AppState>, axum::extract::Path(id): axum::extract::Path<String>) -> impl IntoResponse {
    let _user = match require_admin(&jar, &state).await { Some(u) => u, None => return (StatusCode::FORBIDDEN, "Admin only").into_response() };
    match state.db.execute("DELETE FROM notes WHERE id::text = $1", &[&id]).await {
        Ok(1) => (StatusCode::OK, "Deleted").into_response(),
        _ => (StatusCode::NOT_FOUND, "Not found").into_response()
    }
}
...
async fn admin_delete_upload(jar: CookieJar, state: State<AppState>, axum::extract::Path(id): axum::extract::Path<String>) -> impl IntoResponse {
    let _user = match require_admin(&jar, &state).await { Some(u) => u, None => return (StatusCode::FORBIDDEN, "Admin only").into_response() };
    match state.db.query_opt("SELECT filename FROM uploads WHERE id::text = $1", &[&id]).await {
        Ok(Some(r)) => {
            let filename: String = r.get("filename");
            let path = format!("{}/{}", UPLOADS_DIR, filename);
            let _ = tokio::fs::remove_file(&path).await;
            match state.db.execute("DELETE FROM uploads WHERE id::text = $1", &[&id]).await {
                Ok(1) => (StatusCode::OK, "Deleted").into_response(),
                _ => (StatusCode::NOT_FOUND, "Not found").into_response(),
            }
        }
...
async fn main() -> anyhow::Result<()> {
    ...
    let db_links_url = std::env::var("W9_LINKS_DB_URL").unwrap_or_else(|_| "postgres://w9_admin:password@w9-postgres:5432/w9_links".into());
    ...
    tokio::spawn(async move { if let Err(e) = conn_main.await { tracing::error!("DB main error: {}", e); } });
    ...
    tokio::spawn(async move { if let Err(e) = conn_links.await { tracing::error!("DB links error: {}", e); } });
    ...
    let _ = client_main.execute("CREATE INDEX IF NOT EXISTS idx_links_owner ON links(owner_email)", &[]).await;
    ...
    let state = AppState {
        ...
        http_client: reqwest::Client::builder().timeout(std::time::Duration::from_secs(10)).build()?,
    };
    ...
    let router = Router::new()
        ...
        .route("/n/:code", get(view_notepad)).route("/n/:code/unlock", axum::routing::post(unlock_notepad))
        ...
        .route("/api/admin/links/:id", axum::routing::delete(admin_delete_link))
        .route("/api/admin/notepads/:id", axum::routing::delete(admin_delete_notepad))
        .route("/api/admin/uploads/:id", axum::routing::delete(admin_delete_upload))
        // Health
